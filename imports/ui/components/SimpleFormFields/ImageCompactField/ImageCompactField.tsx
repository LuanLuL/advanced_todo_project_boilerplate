	...
	return (
		<div
			...
			{(!actualImage || actualImage === '-' || !hasValue(actualImage)) && !image && !readOnly ? (
				<label htmlFor={`imageInput${name}`} style={{ width: '100%', marginBottom: '10px' }}>
					<div>
						<div
							style={{
							...
							<div
								...
								<div
									...
									<div style={{ textAlign: 'center' }}>
										<Typography variant="body1">{'+ Adicionar foto de usuário'}</Typography>
									</div>
								</div>
							</div>
						</div>
			...
			<div
				...
				{!readOnly && ((!!actualImage && actualImage !== '-' && hasValue(actualImage)) || !!image) ? (
					<Button size="small" startIcon={<DeleteIcon />} onClick={deleteImageCompact}>
						Deletar
					</Button>
				) : null}
			</div>
